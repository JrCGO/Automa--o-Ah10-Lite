...
class RegistroRepP {
  ...
  /**
   * Import employees from text areas
   */
  importEmployees() {
    ...
    if (!nomesText || !cpfsText) {
      window.Helpers.showNotification("Por favor, preencha os nomes e CPFs dos funcionários", "warning")
      return
    }
    ...
    const cpfs = cpfsText
      .split("\n")
      .map((cpf) => window.Helpers.cleanCPF(cpf))
      .filter((cpf) => cpf)

    if (nomes.length !== cpfs.length) {
      window.Helpers.showNotification("A quantidade de nomes deve ser igual à quantidade de CPFs", "error")
      return
    }
    ...
    window.Helpers.showNotification(`${this.funcionarios.length} funcionários importados com sucesso!`, "success")
  }
  ...
  removeEmployee(id) {
    ...
    this.updateEmployeeList()
    this.updateTotalRecords()
    window.Helpers.showNotification("Funcionário removido", "info")
  }
        ...
        (func) => `
            <div class="funcionario-item">
                <div class="funcionario-info">
                    <div class="funcionario-nome">${window.Helpers.escapeHTML(func.nome)}</div>
                    <div class="funcionario-cpf">${window.Helpers.formatCPF(func.cpf)}</div>
                </div>
  ...
  generateRecords() {
    if (this.funcionarios.length === 0) {
      window.Helpers.showNotification("Adicione funcionários antes de gerar registros", "warning")
      return
    }
    ...
    if (!dataHoraInput?.value) {
      window.Helpers.showNotification("Selecione a data e hora inicial", "warning")
      return
    }
    ...
    window.Helpers.showNotification(`${registros.length} registros gerados com sucesso!`, "success")
  }
  ...
  async copyRecords() {
    if (!this.registrosGerados) {
      window.Helpers.showNotification("Gere os registros primeiro", "warning")
      return
    }

    const success = await window.Helpers.copyToClipboard(this.registrosGerados)

    if (success) {
      window.Helpers.showNotification("Registros copiados para a área de transferência!", "success")
      ...
      window.Helpers.showNotification("Erro ao copiar registros", "error")
    }
  }
}

// Create global instance
window.RegistroRepP = new RegistroRepP()
